...
parser.add_argument('--Nodes_connection_distance_save_path',type=str,default='Nodes_connection_distance.csv')
parser.add_argument('--shortest_path_save_path',type=str,default='AB_images_shortest_path_save.csv')
parser.add_argument('--image_class_associated_codes_path',type=str,default='testAB_latent.csv')
...
Nodes_connection_distance_save_file.to_excel(Nodes_connection_distance_save_file.replace('.csv','xlsx'),index=False)
...
graph = read_graph_from_excel(Nodes_connection_distance_save_file.replace('.csv','xlsx'))
...
with open(shortest_path_save_path,"w",newline="") as csvfile:
    ...
    for i in range(len(A_img_name_list)):
        ...
        data_A_CS=image_class_associated_codes_data.loc[image_class_associated_codes_data['img_name'] == A_img_name]
        data_A_CS_numpy=np.array(data_A_CS.values.tolist()[0][1:])

        data_B_CS = image_class_associated_codes_data.loc[image_class_associated_codes_data['img_name'] == B_img_name]
        ...
        A_node=''
        B_node=''
        A_node_codes_center=0
        B_node_codes_center = 0
